...
- Metrics section: live count-up animation for tenants / provinces / cities / transactions, backed by Supabase queries. Data is ISR-cached and revalidated every 5 minutes (`revalidate = 300`), not queried per request.
...
- Screens:
  ...
  - **Products**: menu items only (MENU_ITEM type) with per-variant BOM editor (components grouped by parent variant; shared section for all-variant components; "Copy to all variants" helper), category CRUD, clone, search.
