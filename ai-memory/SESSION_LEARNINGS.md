...
## 2026-03-10 — ERP Dashboard v2: stats, expandable rows, pagination, date filter
...
- Collapse indicator: `▶` / `▼` in first column.

### Pagination
- `txPage` + `txPageSize` (10 | 25 | 50) state on Recent Transactions table.
- Row-count chips rendered in `.erp-table-header-row` → `.erp-table-controls`.
- Pagination bar (‹ / ›) in `.erp-table-pagination` below the table; hidden when only 1 page.
...
## 2026-03-10 — ERP POS: customers, TRANSFER, payment method gating

### Supabase schema (already existed, no migration needed)
- `tenants.enabled_payment_methods TEXT DEFAULT 'CASH,QRIS,UTANG'` — comma-separated string, not booleans
- `transactions.customer_id UUID`, `transactions.customer_name TEXT`, `transactions.debt_status TEXT` — already present
...
- "+ New Customer" form: name (required) + phone → calls `createCustomer()` → dispatches UPSERT → selects new customer.
...
- In Settings (owner only): 4 toggles — CASH (always on), QRIS, TRANSFER, UTANG. Toggle writes updated comma-string to `tenants` via Supabase and dispatches `SET_RESTAURANT`.
...
## 2026-03-10 — ERP POS improvements
...
- **Card fixed height**: `.erp-pos-card` now has `height: 80px; display: flex; flex-direction: column; justify-content: center` — no more stretch-to-fill-row behavior.
- **Clear cart**: "Hapus/Clear" danger button in `.erp-pos-cart-header` appears only when cart has items; calls `setCart([])`.
- **Customer name dialog**: Checkout button now opens a customer name dialog first (`showCustomerDialog` state). User enters name (optional), then clicks Confirm → checkout payment dialog opens. Customer name is shown as a read-only label in the checkout dialog if provided.
...
## 2026-03-10 — AyaKasir Landing Page & ERP Dashboard improvements
...
- Hero: converted to client component `src/components/ayakasir/Hero.tsx` — typing animation cycles through feature words using useEffect + phase state machine (typing → pause → deleting). Words are in `copy.hero.typingWords`.
- Hero copy shape changed: `title` split into `titlePrefix` / `titleSuffix` / `typingWords`; `ctaLearnMore` replaced with `ctaLogin`.
- Feature items updated to 6-step flow: Purchasing → Inventory → Menu & Products → Customers & Debt → Cashier (POS) → Dashboard & Reports.
...
- CSS: `.ayakasir-hero-inner` flex row, `.ayakasir-typing-word` blue italic, `.ayakasir-cursor` blink animation, `.ayakasir-features-flow` vertical steps grid, `.ayakasir-sync-badge` pill, `.ayakasir-crossplatform` block — all in `globals.css`.

### ERP Dashboard
- ERP shell: `height: 100vh; overflow: hidden` — no page scroll; `.erp-main` is the scrollable zone.
...
- Watermark: `<div class="erp-watermark">AyaKasir by Petalytix | 2026</div>` — fixed bottom-right, muted, pointer-events none.
- NavBar and Footer of the marketing site are not shown in ERP (separate layout with no nav/footer — unchanged, already was so).

## 2026-03-11 — ERP Products/Purchasing restructure

- **ProductsScreen** now MENU_ITEM only — Raw Materials tab removed. Category CRUD locked to `"MENU"` type. No tab selector.
- **PurchasingScreen** gains a 3rd tab "Raw Materials" — full CRUD for RAW_MATERIAL products + RAW_MATERIAL categories.
...
- **BOM unit** (ProductsScreen) and **Goods Receiving unit** (PurchasingScreen) are now read-only `<span class="erp-bom-unit-label">` — auto-filled from inventory on product select.
- New CSS: `.erp-bom-unit-label` in `erp.css`.

## 2026-03-11 — ERP Products: Variants + BOM in product form
...
- `FormVariant` shape: `{ id, name, price_adjustment }` — `price_adjustment` maps to `DbVariant.price_adjustment`.
- `FormComponent` shape: `{ id, component_product_id, required_qty, unit }` — unit auto-filled from inventory when a raw material is selected.
...
## 2026-03-11 — ERP Purchasing page improvements

- **Raw Materials tab**: vendor table no longer shown — fixed a `ternary → rawMaterials tab` fallthrough by replacing `tab === "receiving" ? ... : <vendors>` with two separate `&&` blocks.
- **Receiving form — vendor select**: added `"__NEW__"` sentinel option at the bottom of the dropdown. Selecting it opens the Vendor form dialog; on save the new vendor is auto-selected in the receiving form.
- **Receiving form — raw material select**: default changed from first raw material to `""` (—). Added `"__NEW__"` sentinel option; selecting it opens the Raw Material form; on save the new raw material is auto-selected in the triggering row via `pendingRawItemIdx` state.
- **Duplicate name prevention**: case-insensitive check (`toLowerCase()`) on vendor save and raw material save — alerts and aborts if a name collision exists (excluding the record being edited).
- New i18n keys in `purchasing`: `newVendorOption`, `newRawMaterialOption`, `duplicateVendor`, `duplicateRawMaterial`.

## 2026-03-11 — ERP POS: BOM-aware inventory deduction on sale

- `handleCheckout` in `PosScreen.tsx` now deducts inventory after a successful transaction.
- Logic mirrors mobile app: if a cart item has BOM components (`state.productComponents`), deduct each raw material's stock by `required_qty × item.qty`; otherwise deduct the menu item's own inventory.
- Deductions are accumulated in a `Map` before writing to Supabase — prevents duplicate calls when the same raw material appears in multiple cart item BOMs.
- Uses existing `adjustInventory(supabase, productId, variantId, newQty)` repository function.
- **Unit conversion (follow-up)**: BOM qty and inventory qty normalized to base unit before arithmetic: `kg→g` (×1000), `L→mL` (×1000), `pcs` unchanged. Result converted back to inventory's stored unit before writing. e.g. BOM: 50 mL, inventory stored as L → deduct 0.05 L → 2 L becomes 1.95 L.
- Accumulation in base units — multiple cart items using same raw material combined into one write.
...
- Inventory floor `Math.max(0, current - deduction)` — never negative.
...
## 2026-03-12 — ERP Purchasing: inventory reversal on delete

- `handleDeleteReceiving` in `PurchasingScreen.tsx` now reverses inventory when a goods receiving record is deleted.
- Mirrors the same `toBaseQty` (kg→g, L→mL) + `Math.max(0, current - received)` logic used in the edit-reversal path.
- Iterates `state.goodsReceivingItems` filtered by `receiving_id` before the DB delete — uses local state so no extra Supabase fetch needed.
...
- Order of operations: reverse inventory → delete header (cascades items in DB) → dispatch state cleanup → delete COGS ledger entry.

## 2026-03-12 — ERP Inventory: stock adjustments now recorded in inventory_movements
...
- `movement_type` values align with mobile app: `"adjustment_in"` (Stok Berlebih / Stock Surplus), `"adjustment_out"` (Stok Berkurang / Stock Shortage), `"waste"` (Kadaluwarsa / Rusak — Expired / Damaged). Other types from mobile (e.g. `sale`, `purchase`) may also appear.
...
- `DbInventoryMovement` type in `src/lib/supabase/types.ts`: `variant_id` is `string` (not nullable), `reason` is `string` (not nullable) — matches DB defaults.
...
- `ADJUSTMENT` type remains in `DbGeneralLedger` union — still used by mobile app; only web ERP adjustment path changed.

## 2026-03-13 — ERP Inventory: avg_cogs HPP tracking on stock adjustments
...
- **InventoryScreen**: replaced the client-side `avgCogsMap` (computed from goods receiving history) with `inv.avg_cogs` directly from the DB row — single source of truth.
- **HPP rules on stock adjustment:**
  - `adjustment_in` (bonus units, no cost): `newAvg = (oldAvg × oldQty) / newQty` — existing cost spread over more units → lower HPP.
  - `adjustment_out` / `waste` (shrinkage/spoilage): same formula — existing cost / fewer remaining units → higher HPP.
  - Stock = 0 after adjustment: `avg_cogs` reset to 0.
- **PurchasingScreen goods receiving (apply):** weighted-average formula: `newAvg = (existingAvg × existingQty + costPerBase × receivedBase) / newTotalQty`.
- **PurchasingScreen goods receiving (reversal on edit/delete):** back-calculates pre-purchase avg: `recoveredAvg = (currentAvg × currentQty − costPerBase × oldBase) / newQtyBase`; 0 if result ≤ 0 or newQtyBase = 0.
- New raw material creation: `avg_cogs: 0` in initial inventory row.

## 2026-03-12 — ERP Purchasing: decimal qty in goods receiving

- Qty input now supports decimals for kg/L items (e.g. 1.5 kg, 0.75 L).
- `qty` in `updateRecItem` no longer passes through `parseNum` (which stripped non-digits) — stored as raw string.
- Qty input: `inputMode="decimal"`, `value={item.qty}` (no `fmtNum` formatting). `costPerUnit` formatting unchanged.
- `handleSaveReceiving`: form qty parsed with `parseFloat(i.qty.replace(",", "."))`, then immediately converted to base unit via `toBaseQty` before building the DB row — `Math.round(baseQty)` ensures integer stored in DB.
- `unit` stored in DB is always the base unit (g/mL/pcs), never kg/L — avoids `22P02 invalid input syntax for type integer` error.
- `cost_per_unit` divided by `baseQty` (not raw display qty) for correct per-unit cost.
- `openEditReceiving`: base→display conversion on load (g÷1000→kg, mL÷1000→L) so edit form shows human-friendly values.
- Downstream `toBaseQty` calls in inventory apply/reversal loops receive already-base values and pass through unchanged.

## 2026-03-09 — AyaKasir Desktop ERP implementation
...
- DB types in `src/lib/supabase/types.ts` — matches Supabase schema exactly (snake_case, BIGINT timestamps).
- 10 repository files in `src/lib/supabase/repositories/` — one per entity, function-based (not classes).
- Realtime hook in `src/lib/supabase/realtime.ts` — subscribes to all 12 tenant tables + `tenants` via single channel.
...
- Server-side initial data load in `(erp)/layout.tsx` — fetches all 12 tables + restaurant + user via Supabase server client.
- Realtime sync via `useRealtimeSync` — on INSERT/UPDATE dispatches UPSERT, on DELETE dispatches DELETE.
- Inventory uses composite key (product_id + variant_id) — special handling in reducer.

### ERP Layout
- Route: `src/app/ayakasir/[locale]/app/` — separate from marketing site (no NavBar/Footer).
...
### Design decisions
- Fresh types (not reusing simulator types) — simulator uses camelCase + genId, ERP uses snake_case matching Supabase schema.
...
## 2026-03-13 — ERP Purchasing: float qty bug fix in goods receiving
...
- Pattern: **any value going into `upsertInventory` or `adjustInventory`** — both `current_qty` and `avg_cogs` — must pass through `Math.round()`. Both are BIGINT in Supabase; floats cause `22P02`.
...
## 2026-03-13 — ERP Purchasing: stale inventory snapshot bug fix
...
## 2026-03-13 — ERP Inventory: direct DB correction for corrupted Baterai stock
...
- Correct state per user: `current_qty = 1, avg_cogs = 31` (6 pcs sold out → 0, then 1 pcs received @ Rp31).
...
## 2026-03-13 — ERP Purchasing: vendor required validation on goods receiving
...
## 2026-03-13 — ERP Settings: Business rename + Manajemen User + Pengaturan QRIS
...
- "Configure QRIS" / "Atur QRIS" button opens a dialog to edit merchant name + image URL (paste public URL — no upload from web).
- Saves via new `updateQrisSettingsAction` server action in `auth.ts` → updates `tenants` table, dispatches `SET_RESTAURANT`.
...
- `users` added to `tableKeyMap` in `store.tsx` (realtime key `"users"` → state key `"tenantUsers"`).
...
## 2026-03-13 — ERP Settings: User email validation + Feature Access per Cashier

### Email validation
- `handleSaveUser` in `SettingsScreen.tsx` now checks `email.includes("@")` before calling the server action — shows inline error if invalid, never calls the action.

### Feature Access (Akses Fitur) for CASHIER users
- User form now includes `featureAccess: UserFeature[]` state field.
- Features mirror `UserFeature` enum in the Android app: `POS`, `DASHBOARD`, `MENU`, `INVENTORY`, `PURCHASING`, `CUSTOMERS`, `SETTINGS` — stored as comma-separated string in `users.feature_access`.
...
- When role selector switches CASHIER → OWNER (or back), `featureAccess` is reset appropriately.
...
- Transaction history list (date/time, payment method badge, debt status badge, total) — filtered to `COMPLETED` status, sorted newest first.

### Category management
- CRUD for `customer_categories` with duplicate name check (case-insensitive).
- Delete category does NOT delete customers — `ON DELETE SET NULL` on FK (mirrors Android app).
...
- `.erp-chip` / `.erp-chip--active` — pill filter chips (reusable)
- `.erp-badge--sm`, `.erp-badge--warning`, `.erp-badge--success` — badge variants
- `.erp-table-row--clickable`, `.erp-table-row--selected` — interactive table rows
...
## 2026-03-13 — ERP Cashier role restrictions (Purchasing, Products, Customers)

- CASHIER users see restricted UI in three screens — determined by `const isOwner = state.user?.role === "OWNER"` at the top of each screen.
...
## 2026-03-13 — ERP Settings: CSV Export — date range picker + enriched columns
...
- `customer_name` pulled from `transactions.customer_id → customers.name`; falls back to `tx.notes` if no linked customer (for UTANG).
